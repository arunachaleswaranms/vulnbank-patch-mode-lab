...
## Patch Mode (defensive demo)

The dashboard has a **Patch Mode** switch (global + per-level L1–L5). It's
**OFF by default** — the lab stays fully vulnerable. Turn it **ON** to enable
real server-side security controls so the same attack payloads fail safely:
canaries are redacted, prompt-injection/SQLi is refused, unsafe memory is
dropped, and unsigned agent-to-agent transfers are rejected.

Switching is **live** from the dashboard (no restart). See
[`SECURITY_FIXES.md`](SECURITY_FIXES.md) for the per-level root cause, fix, and
manual verification steps. Boot-time default is set with `PATCH_MODE=off|on`.

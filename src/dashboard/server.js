...
import { getBankProfile, getPatchState, setPatchGlobal, setPatchLevel } from '../bank/profile.js';
import { getPatchAudit } from '../bank/patch.js';
...
export function createDashboardServer({ stats, attackLog, challengeState, agents, logAttack, sandbox, teamName, timerMinutes }) {
  ...
  const server = http.createServer(async (req, res) => {
    ...
    // VulnBank workshop: Patch Mode state (dashboard toggle). GET returns
    // the current global + per-level state; POST sets it live (no restart).
    if (req.method === 'GET' && pathname === '/patch-mode') {
      res.writeHead(200, { 'Content-Type': 'application/json' });
      res.end(JSON.stringify({ ...getPatchState(), audit: getPatchAudit().slice(-20) }, null, 2));
      return;
    }
    if (req.method === 'POST' && pathname === '/patch-mode') {
      try {
        const body = await parseBody(req);
        // { global: 'on'|'off'|'auto' } sets/clears the global override.
        // { level: 1..5, state: 'on'|'off'|'auto' } sets one level.
        let state;
        if (body && (body.level !== undefined && body.level !== null)) {
          state = setPatchLevel(body.level, body.state);
        } else if (body && 'global' in body) {
          state = setPatchGlobal(body.global);
        } else {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'provide { global } or { level, state }' }));
          return;
        }
        res.writeHead(200, { 'Content-Type': 'application/json' });

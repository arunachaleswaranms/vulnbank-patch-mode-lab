...
import { verifyGroqKey, fetchBankStatus, getLLMStatus, disableLLM, evaluateAttack,
  getPatchMode, setPatchGlobal, setPatchLevel } from '../api.js';
...
// Patch Mode control panel: a visible global ON/OFF switch plus per-level
// (L1–L5) toggles, wired to the server-side /patch-mode API. OFF (default)
// keeps the original vulnerable behavior; ON enables the hardened, patched
// controls. State is server-authoritative — this only reflects/sets it.
function buildPatchPanel() {
  const panel = el('div', { className: 'bank-patch-panel', id: 'bank-patch-panel' });

  const head = el('div', { className: 'bank-patch-head' });
  const title = el('div', { className: 'bank-patch-titlewrap' });
  title.appendChild(el('span', { className: 'bank-patch-title' }, 'Patch Mode'));
  const badge = el('span', { className: 'bank-patch-badge off', id: 'bank-patch-badge' }, 'OFF');
  title.appendChild(badge);
  head.appendChild(title);

  const globalBtn = el('button', { className: 'btn bank-patch-global', id: 'bank-patch-global' }, 'Turn Patch Mode ON');
  head.appendChild(globalBtn);
  panel.appendChild(head);

  panel.appendChild(el('p', { className: 'bank-patch-desc', id: 'bank-patch-desc' },
    'OFF — original vulnerable lab: every attack still works. ' +
    'ON — server-side defenses active: the same payloads fail safely (canaries redacted, injections refused, unsafe memory dropped, unsigned A2A transfers rejected).'));

  const levelsRow = el('div', { className: 'bank-patch-levels' });
  const LEVEL_LABELS = {
    1: 'L1 · Prompt Injection', 2: 'L2 · RAG Exfil', 3: 'L3 · SQLi',
    4: 'L4 · Memory', 5: 'L5 · A2A Spoof',
  };
  for (let lvl = 1; lvl <= 5; lvl++) {
    const chip = el('button', { className: 'bank-patch-lvl', id: `bank-patch-lvl-${lvl}` },
      el('span', { className: 'bank-patch-lvl-name' }, LEVEL_LABELS[lvl]),
      el('span', { className: 'bank-patch-lvl-state', id: `bank-patch-lvl-state-${lvl}` }, 'OFF'));
    chip.addEventListener('click', async () => {
      chip.disabled = true;
      const on = chip.classList.contains('on');
      try { applyPatchState(await setPatchLevel(lvl, on ? 'off' : 'on')); }
      catch (e) { /* leave state as-is on error */ }
      chip.disabled = false;
    });
    levelsRow.appendChild(chip);
  }
  panel.appendChild(levelsRow);

  globalBtn.addEventListener('click', async () => {
    globalBtn.disabled = true;
    const anyOn = badge.classList.contains('on');
    try { applyPatchState(await setPatchGlobal(anyOn ? 'off' : 'on')); }
    catch (e) { /* ignore */ }
    globalBtn.disabled = false;
  });

  // Initial load.
  getPatchMode().then(applyPatchState).catch(() => {});
  return panel;
}

// Reflect server-reported patch state onto the panel controls.
function applyPatchState(state) {
  if (!state) return;
  const eff = state.effective || {};
  const anyOn = !!state.anyOn;
  const allOn = !!state.allOn;

  const badge = document.getElementById('bank-patch-badge');
  if (badge) {
    badge.textContent = anyOn ? (allOn ? 'ON' : 'PARTIAL') : 'OFF';
    badge.className = `bank-patch-badge ${anyOn ? 'on' : 'off'}`;
  }
  const globalBtn = document.getElementById('bank-patch-global');
  if (globalBtn) {
    globalBtn.classList.toggle('on', anyOn);
    globalBtn.textContent = allOn ? 'Turn Patch Mode OFF' : 'Turn Patch Mode ON';
  }
  const panel = document.getElementById('bank-patch-panel');
  if (panel) panel.classList.toggle('active', anyOn);

  for (let lvl = 1; lvl <= 5; lvl++) {
    const on = !!eff[lvl];
    const chip = document.getElementById(`bank-patch-lvl-${lvl}`);
    const st = document.getElementById(`bank-patch-lvl-state-${lvl}`);
...
function renderPager() {
  ...
  pager.appendChild(buildPatchPanel());
